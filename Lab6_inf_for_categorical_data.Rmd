...
I filtered out the NAs and did not drive rows and then using count function, I was able to create the drive_text table with all categories in the tibble seen below
```{r}
drive_texting <- yrbss |> 
  filter(!is.na(text_while_driving_30d)) |> 
  filter(!str_detect(text_while_driving_30d, "did not drive")) |> 
...
**Insert your answer here**
The proportion of people who text and drive every day in the past 30 days and never wear helmets is 0.07 (7%) or 463/6503. 

```{r no helmet and drive texting}
...
no_hel <- yrbss %>%
  ...
  count(text_ind) |> 
  mutate(total = sum(n), prop_textdrive_nohelm = (n/total))
...
**Insert your answer here**
ME or margin of error based on the survey is 0.00623
```{r}
set.seed(606)
...
ci |> mutate(ME = ((upper_ci - lower_ci)/2))
```
...
yrbbss_physical <- yrbss |> 
  mutate(PA_active_7d = ifelse(physically_active_7d == "7", "yes", "no")) 
