...
A streak length of 1 means that 1 shot made it into the basket or Hit in this case before missing any shots. A streak length of 0 means that no shots were made and after multiple shots, none went into the basket or no Hits and only consecutive misses. 
...
According to the bar graph displaying all 133 shots made by Kobe, most of his shots are misses as the tallest bar was 0 length where 40 count shots were made and were misses. Noticeably, his most successful streak was 1 with 24 count shots making it into streak 1. As the number of streak goes up, the count number decreases which makes the distribution to be right-skewed. The longest streak he made was 4 where he made 1 count of 4 shots consecutively. 
...
In my unfair coin simulation, only 15 flips were heads out of the 100 flips.

```{r}
set.seed(3126)   #seed set
coin_outcomes <- c("heads", "tails")
...
There needs to be a change in the probability function "prob", the hits vector number needs to be changed to 0.45(45%) to reflect the assigned shooting percentage. 
...
The simulated independent shooter streak lengths graph is very right skewed. In additon the 0 streak miss shots is at 39. The player's longest streak of baskets in 133 shot was at the lenght of 4, 4 consecutive shots in 1 streak. 

```{r}
ggplot(sim_streak, aes(x = length))+
  geom_bar()+
  labs(title = "Simulated independent shooter streak")
```
...
Because I used the set.seed function, It would be the same if I run the simulation again. However, if the set.seed function was removed, I would expect the new streak distribution to be somewhat similar with the order of misses and hits to be shuffled. Due to the probability of missing (55%) and chance of hits (45%) being set, this underlying rule of the simulation will still be true and will be reflected in the bar graph where the characteristic will be right skewed, most common and highest streak is 0 and the next most common streak is 1 with the length gradually decreasing as the streak length gets higher. The shape distribution will be consistent across different runs despite shuffled results.
...
**Insert your answer here**
  ...
  The distribution of streak length between Kobe Bryant and the simulated player are very similar. Both graphs are right skewed, having the longest streak being 0, most common being 1, and the streak of 3-4 are rare. Based o on the data collected, there is not enough evidence to say that Kobe's shooting patterns match the hot hand model. Hot hand theory would make the data dependent and with every shot, there should be an increase probability of making the next shot but that is not true base on his shot outcomes. 
  The simulated player outcomes shows an independent shooter and disregards the outcome of the previous shot. Kobe Bryant's streak distribution is very similar to the simulated player's graph which means the chance of him making a shot each time has a random chance and is independent. 
